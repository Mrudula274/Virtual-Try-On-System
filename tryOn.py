...
def get_face_boundbox(points, face_part):
    global x, y, w, h
    if face_part == 1:  # Left ear
        (x, y, w, h) = calculate_boundbox(points[1:4])  # Adjust indices for the left ear
    elif face_part == 2:  # Right ear
        (x, y, w, h) = calculate_boundbox(points[13:16])
    elif face_part == 3:
...
def cvloop(run_event):
    ...
    while run_event.is_set():
        ...
        for face in faces:
            ...
            is_mouth_open = (shape[66][1] - shape[62][1]) >= 10  # y coordinates of landmark points of lips

            if SPRITES[0]:
                apply_sprite(image, image_path, w, x, y + 40, incl, ontop=False)

            if SPRITES[1]:
                (x1, y1, w1, h1) = get_face_boundbox(shape, 6)
                apply_sprite(image, image_path, w1, x1, y1 + 275, incl)

            if SPRITES[2]:  # Display earrings on both ears
                print(image_path)
                (x2_l, y2_l, w2_l, h2_l) = get_face_boundbox(shape, 1)
                y_offset = 40
                earring_size_multiplier = 20
                apply_sprite(image, image_path, w2_l + earring_size_multiplier , x2_l - 10, y2_l + y_offset, incl, ontop=True)
                # Right ear
                (x2_r, y2_r, w2_r, h2_r) = get_face_boundbox(shape, 2)
                apply_sprite(image, image_path, w2_r + earring_size_multiplier , x2_r + 10, y2_r + y_offset, incl, ontop=True)

               # (x2_l, y2_l, w2_l, h2_l) = get_face_boundbox(shape, 3)
                #apply_sprite(image, image_path, w2_l, x2_l - 50, y2_l+100, incl, ontop=True)  # Left earring
                #(x2_r, y2_r, w2_r, h2_r) = get_face_boundbox(shape, 3)
                #apply_sprite(image, image_path, w2_r, x2_r + 130, y2_r+100, incl, ontop=True)  # Right earring
            (x0, y0, w0, h0) = get_face_boundbox(shape, 6)

            if SPRITES[3]:  # Display tiara
                (x3, y3, _, h3) = get_face_boundbox(shape, 3)
                y_offset = -40
                apply_sprite(image, image_path, w, x, y3 + h3 + y_offset , incl, ontop=True)
                #(x3, y3, _, h3) = get_face_boundbox(shape, 3)
                #apply_sprite(image, image_path, w, x, y3 + h3, incl, ontop=True)

                #(x3, y3, _, h3) = get_face_boundbox(shape, 1)
                #apply_sprite(image, image_path, w, x, y3 + 120, incl, ontop=False)

            (x0, y0, w0, h0) = get_face_boundbox(shape, 6)  # bound box of mouth

            if SPRITES[4]: #top
                (x3, y3, _, h3) = get_face_boundbox(shape, 3)
                # Adjust the y-coordinate to place the top below the neck
                y_offset = y3 + h3 + 110  # You can adjust the offset as needed
                apply_sprite(image, image_path, w, x, y_offset, incl, ontop=True)


            if SPRITES[6]: # Displaying goggles
                (x3, y3, _, h3) = get_face_boundbox(shape, 3)
                y_offset = 50
                apply_sprite(image, image_path, w, x, y3 + h3 + y_offset, incl, ontop=True)
